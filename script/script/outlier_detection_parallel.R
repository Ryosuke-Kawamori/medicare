...
#healthcare <- healthcare %>% sample_n(100000)
#enforce_state <- "massachusetts"
...
cluster <- create_cluster(31)
cluster_library(cluster, c("tidyverse", "keras", "e1071", "DMwR", "dbscan"))
cluster_copy(cluster, scale_this)
cluster_copy(cluster, lof_df)
cluster_copy(cluster, featurize_provider_all)
cluster_copy(cluster, featurize_provider_all_without_providertype)
cluster_copy(cluster, featurize_provider_hcpcs)
cluster_copy(cluster, featurize_provider_stat)
cluster_copy(cluster, featurize_provider_stat_without_providertype)
cluster_copy(cluster, svm_df)
cluster_copy(cluster, leie)
cluster_copy(cluster, min_pts)
cluster_copy(cluster, one_svm)
...
splited_df_provider <- healthcare %>%
  ...
  group_by(provider_type) %>% 
  nest() %>% 
  partition(provider_type, cluster=cluster) %>% 
...
print("provider split completed")
...
splited_df_providermesh <- healthcare %>%
  ...
  group_by(mesh, provider_type) %>% 
  nest() %>% 
  partition(mesh, provider_type, cluster = cluster) %>% 
  dplyr::mutate(hcpcs = purrr::map(data, featurize_provider_hcpcs)) %>%
  dplyr::mutate(stat = purrr::map(data, featurize_provider_stat_without_providertype)) %>% 
  dplyr::mutate(npi = purrr::map(hcpcs, ~as_tibble(rownames(.)))) %>% 
  dplyr::mutate(n = purrr::map_dbl(hcpcs, nrow)) 

print("provider mesh split completed")
...
splited_df_mesh <- healthcare %>%
  ...
  group_by(mesh) %>% 
  nest() %>% 
  partition(mesh, cluster=cluster) %>% 
  dplyr::mutate(hcpcs = purrr::map(data, featurize_provider_hcpcs)) %>%
  dplyr::mutate(stat = purrr::map(data, featurize_provider_stat_without_providertype)) %>% 
  dplyr::mutate(npi = purrr::map(hcpcs, ~as_tibble(rownames(.)))) %>% 
  dplyr::mutate(n = purrr::map_dbl(hcpcs, nrow)) 

print("mesh split completed")

# provider stat feature ---------------------------------------------------

lof_provider_stat_result <- lof_df_parallel(df=splited_df_provider,
...
lof_provider_hcpcs_result <- lof_df_parallel(df=splited_df_provider,
                                    group_name = quos(provider_type),
                                    feature_name = quo(hcpcs),
                                    min_pts = min_pts,
                                    leie=leie)

print("lof proivder completed")

# provider-mesh stat feature -------------------------------------------------------

lof_providermesh_stat_result <- lof_df_parallel(df=splited_df_providermesh,
...
lof_providermesh_hcpcs_result <- lof_df_parallel(df=splited_df_providermesh,
                                        group_name = quos(provider_type, mesh),
                                        feature_name = quo(hcpcs),
                                        min_pts = min_pts,
                                        leie=leie) 
...
print("lof proivder mesh completed")


# Mesh stat feature -------------------------------------------------------

lof_mesh_stat_result <- lof_df_parallel(df=splited_df_mesh,
...
lof_mesh_hcpcs_result <- lof_df_parallel(df=splited_df_mesh,
                                group_name = quos(mesh),
                                feature_name = quo(hcpcs),
                                min_pts = min_pts,
                                leie=leie) 
...
print("lof mesh completed")
# result of lof -----------------------------------------------------------

result_lof_df <- tibble(type = c("none stat", "provider stat", "provider hcpcs", "provider-mesh stat", "provider-mesh hcpcs", "mesh stat", "mesh hcpcs"), 
...
write_rds(lof_provider_stat_result, "data/lof_provider_stat_result.rds")
write_rds(lof_provider_hcpcs_result, "data/lof_provider_hcpcs_result.rds")
write_rds(lof_providermesh_stat_result, "data/lof_providermesh_stat_result.rds")
write_rds(lof_providermesh_hcpcs_result, "data/lof_providermesh_hcpcs_result.rds")
write_rds(lof_mesh_stat_result, "data/lof_mesh_stat_result.rds")
write_rds(lof_mesh_hcpcs_result, "data/lof_mesh_hcpcs_result.rds")

# one-svm provier stat feature-----------------------------------------------------------
# svm_provider_stat_result <- svm_df(splited_df_provider, 
#                                    group_name = quos(provider_type), 
#                                    feature_name = quo(stat),
#                                    nu = nu_range,
#                                    leie = leie)
...
# svm_provider_hcpcs_result <- svm_df(splited_df_provider,
#                                     group_name = quos(provider_type),
#                                     feature_name = quo(stat),
#                                     nu = nu_range,
#                                     leie = leie)
...
# 
# 
# svm_providermesh_stat_result <- svm_df(splited_df_providermesh,
#                                         group_name = quos(provider_type, mesh),
#                                         feature_name = quo(stat),
#                                         nu = nu_range,
#                                         leie = leie)
...
# 
# 
# svm_proviermesh_hcpcs_result <- svm_df(splited_df_providermesh,
#                                        group_name = quos(provider_type, mesh),
#                                        feature_name = quo(hcpcs),
#                                        nu = nu_range,
#                                        leie = leie)
...
# 
# 
# svm_mesh_stat_result <- svm_df(splited_df_mesh,
#                                group_name = quos(mesh),
#                                feature_name = quo(stat),
#                                nu = nu_range,
#                                leie = leie)
...
# 
# 
# svm_mesh_hcpcs_result <- svm_df(splited_df_mesh,
#                                 group_name = quos(mesh),
#                                 feature_name = quos(hcpcs),
#                                 nu = nu_range,
#                                 leie = leie)
